...
class Nasabah_model extends ci_model{
  ...
  var $table = 'nasabah';
    ...
    var $order = array('id_nasabah' => 'desc'); // default order 
    ...
    private function _get_datatables_query()
    {
      ...
      $this->db->join('agunan as a', 'a.id_agunan = b.agunan');
    ...
    function dataperuser()
    {
        ...
        $this->db->order_by('id_nasabah','DESC');
        $this->db->where('nasabah.user_id', $this->session->userdata('login_session')['id_user']);
        return $query = $this->db->get('nasabah');
    }
    ...
    public function dataJoin()
    {
      ...
      $this->db->order_by('b.id_nasabah','DESC');
    ...
    public function detail_join($where)
    {
      $this->db->select('*');
      $this->db->from('nasabah as b');
      $this->db->where('b.id_nasabah', $where);
    ...
    public function ambil_stok($where){
      $this->db->order_by('id_nasabah','DESC');
      $this->db->limit(1);
      $query = $this->db->get_where('nasabah',$where);
      $data = $query->row();
      $stok = $data->stok;
      return $stok;
    }
