...
class Penagihan_model extends ci_model{
    ...
    var $table = 'penagihan';
    var $column_order = array(null, 'tanggal_penagihan','nama','id_kantor', 'kolektabilitas',null, 'nama_nasabah', null, 'nominal'); //set column field database for datatable orderable
    var $column_search = array('tanggal_penagihan','nama','nama_nasabah'); //set column field database for datatable searchable 
    var $order = array('tanggal_penagihan' => 'desc'); // default order 
    ...
    private function _get_datatables_query_manajer()
    {
      ...
      $this->db->where('penagihan.kantor_id', $this->session->userdata('login_session')['kantor_id']);
    ...
    private function _get_datatables_query_user()
    {
      ...
      $this->db->where('penagihan.user_id', $this->session->userdata('login_session')['id_user']);
    ...
    private function _get_datatables_querytgluserkantor($tglawal, $tglakhir, $user, $kantor)
    {
      ...
      $this->db->where('p.tanggal_penagihan >=', $tglawal);
      $this->db->where('p.tanggal_penagihan <=', $tglakhir);
    ...
    public function count_alltgluserkantor($tglawal, $tglakhir, $user, $kantor)
    {
        ...
        $this->db->where('tanggal_penagihan >=', $tglawal);
        $this->db->where('tanggal_penagihan <=', $tglakhir);
    ...
    function data()
    {
        $this->db->order_by('id_penagihan','DESC');
        return $query = $this->db->get('penagihan');
    }
    ...
    public function dataJoin()
    {
      ...
      $this->db->order_by('p.id_penagihan','DESC');
    ...
    public function dataJoinLike($tahun)
    {
      ...
      $this->db->like('p.tanggal_penagihan', $tahun);
      $this->db->order_by('p.id_penagihan','DESC');
      return $query = $this->db->get();
    }
    ...
    public function detailJoin($where)
    {
      $this->db->select('*');
      $this->db->from('penagihan as p');
      $this->db->where('id_penagihan', $where);
      $this->db->join('user as u', 'u.id_user = p.user_id');
      $this->db->join('storting as s', 's.id_storting = p.storting_id');
      $this->db->join('kantor as d', 'd.id_kantor = p.kantor_id');
    ...
    public function ambilFoto1($where)
    {
      $this->db->order_by('id_penagihan','DESC');
      $this->db->limit(1);
      $query = $this->db->get_where('penagihan', $where);   
