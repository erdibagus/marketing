...
class Telemarketing_model extends ci_model{
    ...
    var $table = 'telemarketing';
    var $column_order = array(null, 'tanggal_telemarketing','user_id','kpk','kategori_id','sumber_id','nama_nasabah','alamat','usaha','hasil','no_tlp'); //set column field database for datatable orderable
    var $column_search = array('tanggal_telemarketing','nama','nama_nasabah_prospek'); //set column field database for datatable searchable 
    var $order = array('tanggal_telemarketing' => 'desc'); // default order 
    ...
    {
      $this->db->from('telemarketing as p');
      $this->db->join('user as u', 'u.id_user = p.user_id');
      $this->db->join('sumber as k', 'k.id_sumber = p.sumber_id');
    ...
    {
      $this->db->from('telemarketing');
      $this->db->join('user', 'id_user = user_id');
      $this->db->join('sumber', 'id_sumber = sumber_id');
      ...
      $this->db->where('telemarketing.kpk', $this->session->userdata('login_session')['kantor_id']);
    ...
    private function _get_datatables_query_user()
    {
      ...
      $this->db->join('kantor', 'id_kantor = kpk');
      $this->db->join('nasabah_prospek', 'id_nasabah_prospek = nasabah_id');
      $this->db->where('telemarketing.user_id', $this->session->userdata('login_session')['id_user']);
    ...
    private function _get_datatables_querytgluserkantor($tglawal, $tglakhir, $user, $kantor)
    {
      ...
      $this->db->where('p.tanggal_telemarketing >=', $tglawal);
      $this->db->where('p.tanggal_telemarketing <=', $tglakhir);
    ...
    public function count_alltgluserkantor($tglawal, $tglakhir, $user, $kantor)
    {
        ...
        $this->db->where('tanggal_telemarketing >=', $tglawal);
        $this->db->where('tanggal_telemarketing <=', $tglakhir);
    ...
    function data()
    {
        $this->db->order_by('id_telemarketing','DESC');
        return $query = $this->db->get('telemarketing');
    }
    ...
    public function dataJoin()
    {
      ...
      $this->db->from('telemarketing as p');
      $this->db->join('user as u', 'u.id_user = p.user_id');
      $this->db->join('kolektabilitas as k', 'k.id_kolektabilitas = p.kolektabilitas_id');
      $this->db->join('kategori as s', 's.id_kategori = p.kategori_id');
      $this->db->join('kantor as d', 'd.id_kantor = p.kpk');
      $this->db->join('nasabah_prospek as j', 'j.id_nasabah_prospek = p.nasabah_id');

      $this->db->order_by('p.id_telemarketing','DESC');
    ...
    public function dataJoinLike($tahun)
    {
      ...
      $this->db->like('p.tanggal_telemarketing', $tahun);
      $this->db->order_by('p.id_telemarketing','DESC');
      return $query = $this->db->get();
    }
    ...
    public function detailJoin($where)
    {
      $this->db->select('*');
      $this->db->from('telemarketing as p');
      $this->db->where('id_telemarketing', $where);
      $this->db->join('sumber as k', 'k.id_sumber = p.sumber_id');
      $this->db->join('kategori as s', 's.id_kategori = p.kategori_id');
      $this->db->join('user as u', 'u.id_user = p.user_id');
      $this->db->join('kantor as d', 'd.id_kantor = p.kpk');
    ...
    public function ambilFoto1($where)
    {
      $this->db->order_by('id_telemarketing','DESC');
      $this->db->limit(1);
      $query = $this->db->get_where('telemarketing', $where);   
